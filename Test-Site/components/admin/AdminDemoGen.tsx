...
import { Zap, Globe, Play, FileCode, Key, Save, RefreshCw, Plus, Trash2, CheckCircle, Database, LayoutTemplate, Terminal, AlertTriangle, Dice5, AlertCircle, BarChart3, Info } from 'lucide-react';
...
const AdminDemoGen: React.FC<AdminDemoGenProps> = ({ onSuccess }) => {
    ...
    const handleCreateProject = async () => {
        ...
        // Find active Key
        const activeApi = settings.apiConfigs?.find(c => c.id === selectedApiId);
        const apiKeyToUse = activeApi ? activeApi.key : settings.sparkTrafficApiKey;

        if (!apiKeyToUse) {
            setLastError('No valid API Key selected. Please configure an API provider.');
            return;
        }
        ...
        // --- PRODUCTION MODE: Override Size ---
        // Ensure the selected volume from the dropdown overrides whatever is in the JSON template
        parsedSchema.size = selectedVolume;

        setIsCreatingDemo(true);

        try {
            // Call API
            const { id: externalId, raw } = await sparkTrafficService.createProject(demoName, demoUrl, parsedSchema, apiKeyToUse);
            
            setApiResponse(raw); // Store the raw response to display

            if (raw && (raw.error || raw.message)) {
                // If API returns an error inside the JSON
                throw new Error(raw.error || raw.message);
            }

            if (externalId === 'unknown_id' || !externalId) {
                throw new Error("API request succeeded but returned no ID.");
            }

            // Save to DB on success
            const today = new Date();
            // Determine expiry based on plan
            const durationMs = selectedVolume === 'demo' ? (4 * 60 * 60 * 1000) : (30 * 24 * 60 * 60 * 1000); 
            const expiresDate = new Date(today.getTime() + durationMs);
            
            // Map JSON Schema to Local Settings
            const defaultSettings: ProjectSettings = {
                trafficSpeed: 100, 
                bounceRate: parsedSchema.bounce_rate || 0, 
                returnRate: parsedSchema.return_rate || 0, 
                deviceSplit: parsedSchema.desktop_rate || 70,
                deviceSpecific: "All",
                timeOnPage: parsedSchema.time_on_page || '30 seconds', 
                timezone: 'UTC', 
                language: 'en-US',
                languages: ['en-US'], // Default
                gaId: parsedSchema.ga_id || '',
                ...
                geoTargets: [{ id: 'geo-demo', country: 'US', percent: 100 }],
                trafficSource: parsedSchema.traffic_type || 'Direct',
                keywords: parsedSchema.keywords || '', 
                referralUrls: parsedSchema.referrers || parsedSchema.social_links || '',
                sitemap: '', shortener: '',
                autoRenew: parsedSchema.auto_renew === 'true', 
                cacheWebsite: false, minimizeCpu: false, 
                randomizeSession: true, antiFingerprint: true,
                pageViewsWithScroll: parsedSchema.scroll_events || 0, 
                clickExternal: parsedSchema.click_outbound_events || 0, 
                clickInternal: 0
            };
            ...
            const newProject: Project = {
                ...
                name: demoName,
                plan: selectedVolume === 'demo' ? 'Free Trial' : selectedVolume.toUpperCase(),
            ...
            // If response wasn't set (network error), mock an error object for display
            if (!apiResponse) {
