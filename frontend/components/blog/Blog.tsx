...
const Blog: React.FC<BlogProps> = ({ onBack }) => {
  ...
  const updateSEO = (title: string, description: string) => {
      document.title = `${title} | Traffic Creator Journal`;
  ...
  const handleOpenArticle = (id: number) => {
      ...
      if (article) {
          ...
          setView('article');
          updateSEO(article.title, article.seoDescription);
          window.scrollTo(0,0);
      }
  };
  ...
  const handleBack = () => {
      ...
      updateSEO("Blog", "Insights and strategies for traffic growth.");
      window.scrollTo(0,0);
  };
  ...
  const renderSchema = (article: typeof articles[0]) => {
      const schema = {
          "@context": "https://schema.org",
          "@type": "BlogPosting",
          "headline": article.title,
          "image": [article.image],
          "datePublished": new Date(article.date).toISOString(), // Approx for demo
          "author": [{
              "@type": "Person",
              "name": article.author
          }],
