...
import {
    ...
    Plus, Trash2, Download, Upload, AlertCircle, FileCode, Search, MapPin, X, Target, BarChart2, Split
} from 'lucide-react';
...
const ProjectDetails: React.FC<ProjectDetailsProps> = ({ projectId, onBack, onUpdate }) => {
    ...
    useEffect(() => {
        const loadProject = async () => {
            setLoading(true);
            let p = db.getProjectById(projectId);

            // If not in cache, try fetching directly
            if (!p) {
                console.log(`[ProjectDetails] Project ${projectId} not found in cache, fetching from API...`);
                p = await db.fetchProjectById(projectId);
            }
            ...
            if (p) {
                ...
                const defaultSettings: ProjectSettings = {
                    ...
                    pagesPerVisitor: 3,
                    customSubpages: [],
                    sitemapAutoCrawl: false
                };

                // Merge with loaded settings, providing safe fallbacks
                let loadedSettings = p.settings || {};

                // Legacy Migration: if 'url' exists but 'entryUrls' doesn't, map it
                if (loadedSettings.url && !loadedSettings.entryUrls) {
                    loadedSettings.entryUrls = loadedSettings.url;
                }

                let currentSettings = { ...defaultSettings, ...loadedSettings };

                // Migration: Map old countries to geoTargets with ISO if possible
                if ((!currentSettings.geoTargets || currentSettings.geoTargets.length === 0) && (currentSettings.countries && currentSettings.countries.length > 0)) {
                ...
                // Final sanity checks for arrays
                if (!Array.isArray(currentSettings.languages)) {
                    currentSettings.languages = [currentSettings.language || 'en-US'];
                }
                if (!Array.isArray(currentSettings.geoTargets)) {
                    currentSettings.geoTargets = [];
                }
                if (!Array.isArray(currentSettings.customSubpages)) {
                    currentSettings.customSubpages = [];
                }
    ...
    const handleScanGA = async () => {
        ...
        try {
            const result = await db.findTid(entryUrl);
            if (result && result.tid) {
                handleChange('gaId', result.tid);
        ...
        <div className="space-y-6 animate-in fade-in duration-500 pb-20">
            ...
            <div className={isFreeTrial ? 'opacity-70 pointer-events-none grayscale-[0.5]' : ''}>
                ...
                <div className="bg-white border border-gray-200 shadow-sm p-6 md:p-8 mt-6">
                    <h3 className="text-xs font-bold uppercase tracking-widest text-[#ff4d00] mb-8 flex items-center gap-2">
                        <Split size={16} /> Journey & Content
                    </h3>

                    <div className="space-y-8">
                        <RangeControl
                            label="Pages per Visitor"
                            value={settings.pagesPerVisitor || 3}
                            onChange={(v) => handleChange('pagesPerVisitor', v)}
                            max={15}
                            suffix=" Pages"
                        />
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex items-center gap-3">
                                <input
                                    ...
                                    className="flex-1 bg-[#f9fafb] border-b-2 border-transparent focus:border-[#ff4d00] p-3 outline-none text-sm font-medium text-gray-900 transition-colors"
                                    disabled={isFreeTrial}
                                />
                                <div className="flex items-center gap-2 border border-gray-200 p-2 rounded-sm bg-white">
                                    <span className="text-[10px] font-bold text-gray-400 uppercase">Auto-Crawl</span>
                                    <Toggle label="" checked={settings.sitemapAutoCrawl || false} onChange={(v) => handleChange('sitemapAutoCrawl', v)} />
                                </div>
                            </div>
                        </div>

                        <div>
                            <Label>Custom Funnel Steps</Label>
                            <div className="space-y-4 mt-2">
                                {Array.from({ length: settings.pagesPerVisitor || 3 }).map((_, index) => (
                                    <div key={index} className="flex gap-3 items-start">
                                        <span className="text-xs font-bold text-gray-400 w-12 flex-shrink-0 pt-3">Step {index + 1}</span>
                                        <textarea
                                            value={Array.isArray(settings.customSubpages) ? settings.customSubpages[index] || '' : ''}
                                            onChange={(e) => {
                                                const newSubpages = [...(settings.customSubpages || [])];
                                                // Ensure array is long enough
                                                while (newSubpages.length <= index) newSubpages.push('');
                                                newSubpages[index] = e.target.value;
                                                handleChange('customSubpages', newSubpages);
                                            }}
                                            placeholder={index === 0 ? "https://example.com/features" : "https://example.com/pricing"}
                                            className="w-full bg-[#f9fafb] border-b-2 border-transparent focus:border-[#ff4d00] p-3 outline-none text-sm font-medium text-gray-900 transition-colors h-20 resize-y"
                                            disabled={isFreeTrial}
                                        />
                                    </div>
                                ))}
