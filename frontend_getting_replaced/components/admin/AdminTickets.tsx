...
const AdminTickets: React.FC<AdminTicketsProps> = ({ tickets, onRefresh }) => {
    ...
    const selectedProject = selectedTicket?.projectId ? projects.find(p => p.id === selectedTicket.projectId) : null;
    ...
    const sortedTickets = [...tickets].sort((a,b) => {
        ...
        const matchesFilter = activeFilter === 'all' ? true : t.type === activeFilter;
    ...
    const handleSendMessage = async () => {
        ...
        await db.replyToTicket(selectedTicketId, replyText, 'admin', attachments);
        setReplyText('');
        setAttachments([]);
        onRefresh();
    };
                        ...
                        <div className="bg-white p-6 border-b border-gray-200 flex justify-between items-start shadow-sm z-10">
                            <div>
                                ...
                                <div className="flex items-center gap-4 text-xs text-gray-500">
                                    ...
                                    {selectedProject ? (
                                        <span className="text-[#ff4d00] font-bold uppercase">Project: {selectedProject.name}</span>
                                    ) : (
