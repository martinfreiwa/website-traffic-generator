...
import { TrendingUp, Activity, CreditCard, Users, ArrowRight, Clock, ShieldCheck, Zap, Info, AlertTriangle, AlertOctagon, Star, Plus, Play, Pause, Settings, Smartphone, X } from 'lucide-react';
...
const HomeDashboard: React.FC<HomeDashboardProps> = ({ projects, balance, onNavigateToProject, onNavigateToBuyCredits, onNavigateToAddProject, onNavigateToCampaigns }) => {
  ...
  useEffect(() => {
    ...
    // Check for low credit (threshold: €10)
    setShowLowCreditAlert(balance < 10 && balance > 0);
  }, [balance]);
  ...
  const handleQuickAction = (action: string) => {
    switch(action) {
      ...
      case 'view-campaigns':
        ...
        break;
      case 'resume-all':
        // Bulk resume all stopped campaigns
        stoppedProjects.forEach(p => {
          db.updateProjectStatus(p.id, 'active');
        });
        // We'll let the user manually refresh or handle it via a toast, but for now a simple alert/reload is fine
        // window.location.reload(); // Avoid full reload in React if possible
        alert("Resuming all stopped campaigns...");
        break;
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        {showLowCreditAlert && (
            <div className="bg-gradient-to-r from-orange-500 to-red-500 text-white p-4 rounded-sm shadow-lg flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <AlertTriangle size={24} />
                    <div>
                        <p className="font-bold text-sm">Low Credit Balance</p>
                        <p className="text-xs opacity-90">Your balance is €{balance.toFixed(2)}. Add funds to keep your campaigns running.</p>
                    </div>
                </div>
                <button 
                    onClick={onNavigateToBuyCredits}
                    className="bg-white text-orange-600 px-4 py-2 text-xs font-bold uppercase tracking-wider rounded-sm hover:bg-gray-100 transition-colors"
                >
                    Add Funds
                </button>
            </div>
        ...
        {alerts.length > 0 && (
            <div className="space-y-3">
                {alerts.map(alert => (
                    <div key={alert.id} className={`p-4 border-l-4 shadow-sm flex items-start gap-3 rounded-r-sm ${
        ...
        <div className="bg-white border border-gray-200 shadow-sm p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <button 
                    onClick={() => handleQuickAction('new-campaign')}
                    className="flex flex-col items-center gap-2 p-4 bg-gray-50 hover:bg-[#ff4d00] hover:text-white rounded-sm transition-all group border border-gray-100 hover:border-[#ff4d00]"
                >
                    <Plus size={24} className="text-[#ff4d00] group-hover:text-white" />
                ...
                <button 
                    ...
                    className="flex flex-col items-center gap-2 p-4 bg-gray-50 hover:bg-[#ff4d00] hover:text-white rounded-sm transition-all group border border-gray-100 hover:border-[#ff4d00]"
                >
                    <CreditCard size={24} className="text-[#ff4d00] group-hover:text-white" />
                    <span className="text-xs font-bold uppercase tracking-wider">Add Funds</span>
                </button>
                <button 
                    onClick={() => handleQuickAction('view-campaigns')}
                    className="flex flex-col items-center gap-2 p-4 bg-gray-50 hover:bg-[#ff4d00] hover:text-white rounded-sm transition-all group border border-gray-100 hover:border-[#ff4d00]"
                >
                    <Settings size={24} className="text-[#ff4d00] group-hover:text-white" />
                    <span className="text-xs font-bold uppercase tracking-wider">Manage Campaigns</span>
                </button>
                {stoppedProjects.length > 0 && (
                    <button 
                        onClick={() => handleQuickAction('resume-all')}
                        className="flex flex-col items-center gap-2 p-4 bg-gray-50 hover:bg-green-500 hover:text-white rounded-sm transition-all group border border-gray-100 hover:border-green-500"
                    >
                        <Play size={24} className="text-green-500 group-hover:text-white" />
                        <span className="text-xs font-bold uppercase tracking-wider">Resume All ({stoppedProjects.length})</span>
                    </button>
                )}
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-6 border border-gray-200 shadow-sm relative overflow-hidden group hover:border-[#ff4d00] transition-colors cursor-pointer" onClick={onNavigateToBuyCredits}>
             ...
             <div className="bg-white p-6 border border-gray-200 shadow-sm relative overflow-hidden group hover:border-[#ff4d00] transition-colors">
                ...
                <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">System Load</div>
                <div className="text-3xl font-black text-gray-900 mb-1">34%</div>
                <div className="w-full bg-gray-100 h-1.5 mt-2 overflow-hidden rounded-full">
                    <div className="bg-[#ff4d00] h-full w-[34%]"></div>
                </div>
            </div>
        </div>
                             ...
                             {projects.slice(0, 5).map(p => (
                                 <tr key={p.id} className="hover:bg-orange-50/50 cursor-pointer transition-colors group" onClick={() => onNavigateToProject(p.id)}>
                                     ...
                                     <td className="px-6 py-4 text-right">
                                         <span className={`inline-flex items-center px-2 py-0.5 text-[9px] font-black uppercase tracking-wider rounded-sm ${
