...
export interface Target {
  url: string;
  tid: string; // GA4 Tracking ID
  funnel?: Array<{ url: string; title?: string }>;
}
...
export interface ProjectSettings {
  ...
  sitemap: string;
  sitemapAutoCrawl?: boolean; // New: Auto-crawl sitemap every hour
  pagesPerVisitor?: number; // New: 1-15 pages per session
  customSubpages?: string[]; // New: List of custom subpages
  shortener: string;
  autoRenew: boolean;
  ...
  // CRITICAL: targets array for backend traffic generation
  targets?: Target[];
...
export interface User {
  ...
  apiKey?: string;
  api_access_status?: 'none' | 'pending' | 'approved' | 'rejected';
  api_application_data?: {
...
export interface SystemSettings {
  ...
  traffic_enabled?: boolean; // Global pause flag
}
