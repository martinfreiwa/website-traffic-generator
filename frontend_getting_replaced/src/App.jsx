...
import {
  ...
  findTid, bulkCreateProxies, testProxy,
  getStats, getProjects, createProject, deleteProject,
  startTraffic, stopTraffic, testUrl, getProjectLogs,
  login, register, getMe, setAuthToken, getTransactions, getAffiliateEarnings, generateApiKey, API_BASE_URL
} from './api';
...
function App() {
  ...
  const [buyAmount, setBuyAmount] = useState(499);
  ...
  useEffect(() => {
    ...
    let sse = null;
    try {
      sse = new EventSource(`${API_BASE_URL}/admin/live-pulse`);
      sse.onmessage = (e) => {
        try {
          const newLogs = JSON.parse(e.data);
          if (newLogs && newLogs.length > 0) {
            setRecentEvents(prev => [...newLogs, ...prev].slice(0, 50));
          }
        } catch (err) {
  ...
  const handleRegister = async (e) => {
    ...
    setError(null);
    try {
      await register(loginEmail, loginPass);
  ...
  const getVisitsForAmount = (amount) => {
    let rate; // CPM
    if (amount >= 2999) rate = 0.1999;
    else if (amount >= 1249) rate = 0.249;
    else if (amount >= 999) rate = 0.33;
    else if (amount >= 399) rate = 0.40;
    else if (amount >= 129) rate = 0.43;
    else rate = 0.58;

    const visits = Math.floor((amount / rate) * 1000);
    return { visits, rate };
  };

  const { visits: estVisits, rate: estRate } = getVisitsForAmount(buyAmount);

  if (authLoading) return <div className="loading-screen">TrafficGen Pro is loading...</div>;

  if (!user) {
    return (
      <div className="auth-container">
        <div className="auth-card">
          <h2>TrafficGen Pro</h2>
          <p>{authView === 'login' ? 'Sign in to your account' : 'Create a new account'}</p>
          ...
          <button className="btn-link" onClick={() => setAuthView(authView === 'login' ? 'register' : 'login')}>
  ...
  return (
    <div className="dashboard">
      ...
      <div className="tabs">
        ...
        <button className={`tab-btn ${activeTab === 'financials' ? 'active' : ''}`} onClick={() => setActiveTab('financials')}>Wallet & Affiliate</button>
        <button className={`tab-btn ${activeTab === 'developers' ? 'active' : ''}`} onClick={() => setActiveTab('developers')}>Developers</button>
      ...
      {activeTab === 'financials' && (
        <div className="section financials-panel">
          <div className="financials-grid">
            <div className="wallet-card">
              <h3>Balance & Credits</h3>
              <p className="wallet-balance">€{user.balance.toFixed(2)}</p>
              <button className="btn-primary" onClick={() => setShowBuyModal(true)}>Add Funds / Buy Credits</button>
              ...
              <div className="transaction-history">
                ...
                <div className="tx-list">
                  ...
                  {userTransactions.length === 0 && <p className="empty-msg">No transactions yet.</p>}
                </div>
              </div>
            </div>
            ...
            <div className="affiliate-card">
              ...
              <div className="affiliate-stats">
                <div className="affiliate-stat">
                  ...
                  <span className="value">{user.affiliate_code}</span>
                </div>
                <div className="affiliate-stat">
                  <span className="label">Total Earnings</span>
                  <span className="value">€{userEarnings.reduce((acc, curr) => acc + curr.amount, 0).toFixed(2)}</span>
                </div>
              </div>
              <p className="help-text">Earn 20% Lifetime Commission for every user you refer.</p>
              <div className="referral-link-group">
                <input readOnly value={`https://trafficgen.pro/ref/${user.affiliate_code}`} className="referral-input" />
                <button className="btn-mini" onClick={() => { navigator.clipboard.writeText(`https://trafficgen.pro/ref/${user.affiliate_code}`); alert("Copied!"); }}>Copy</button>
              </div>
                ...
                <div className="tx-list">
                  {userEarnings.map(e => (
                    <div key={e.id} className="tx-item">
                      <span className="tx-date">{new Date(e.created_at).toLocaleDateString()}</span>
                      <span className="tx-desc">Referral: {e.referee_email}</span>
                      <span className="tx-amount credit">+€{e.amount.toFixed(2)}</span>
                    </div>
                  ))}
                  {userEarnings.length === 0 && <p className="empty-msg">No commissions earned yet.</p>}
                </div>
              </div>
            </div>
          </div>
      ...
      {activeTab === 'developers' && (
        <div className="section developers-panel">
          <div className="section-header">
            <h3>API Access</h3>
          ...
          <div className="api-key-box">
            ...
            <input readOnly value={user.api_key || 'No key generated yet'} className="api-key-input" />
            <p className="help-text">Use this key in the <code>X-API-KEY</code> header for programmatic access.</p>
          </div>
          <div className="doc-links">
            <h4>Quick Integration</h4>
            <pre className="code-snippet">
              {`curl -X POST "http://localhost:8000/start" \\
     -H "X-API-KEY: ${user.api_key || 'YOUR_KEY'}" \\
     -H "Content-Type: application/json" \\
     -d '{
       "targets": [{"url": "https://example.com", "tid": "G-XXXX"}],
      ...
      {showBuyModal && (
        <div className="modal-overlay">
          <div className="modal-card">
            <h3>Add Credits</h3>
            <p className="help-text">Input an amount to calculate visits. Payments are currently direct (Wise/Revolut).</p>

            <div className="amount-slider">
              <label>Investment Amount: €{buyAmount}</label>
              <input
                type="range"
            ...
            <div className="quote-display">
              <div className="quote-stat">
                <span className="label">Estimated Visits</span>
                <span className="value">{estVisits.toLocaleString()}</span>
              </div>
              <div className="quote-stat">
                <span className="label">CPM Rate</span>
                <span className="value">€{estRate.toFixed(4)}</span>
              </div>
            </div>

            <div className="payment-instructions">
              <h4>Payment Instructions</h4>
              <p>Transfer <strong>€{buyAmount}</strong> to:</p>
              <code>REVOLUT-TAG: @marketing_gen</code>
              <p className="small-text">Include your email <strong>{user.email}</strong> in the reference. Credits are added within 10 mins.</p>
            </div>

            <button className="btn-primary" onClick={() => setShowBuyModal(false)}>I have transferred funds</button>
            <button className="btn-link" onClick={() => setShowBuyModal(false)}>Close</button>
          </div>
        </div>
      )}
    </div>
